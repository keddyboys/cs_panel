...
class Server
{
     ...
     */
    public function __construct(array $server_info = array())
    {
    ...
    protected function checkAndSetIpPort($ip_address)
    {
        ...
        if (substr_count($ip_address, ':') > 1) {
            ...
            // Now let's validate the IPv6 value sent, remove the square brackets (array()) first
            if (!filter_var(trim($this->ip, 'array()'), FILTER_VALIDATE_IP, array('flags' => FILTER_FLAG_IPV6,))) {
