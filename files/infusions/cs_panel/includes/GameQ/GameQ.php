...
class GameQ
{
   ...
   /* protected $options = [
    ...
    protected $options = array( 'debug' => false,
	        'timeout' => '3', 
			'filters' => array(
                'normalize_d751713988987e9331980363e24189ce' => array(
                'filter'  => 'normalize',
                'options' => array(),
                ),
            ),
            'stream_timeout'       => 200000,
            'write_wait'           => 500,
            'capture_packets_file' => null,			
		);	
     ...
     * @return $this
     */
    public function addServer(array $server_info = array())
    {
     ...
     * @throws \Exception
     */
    public function addServersFromFiles($files = array())
    {
     ...
     * @return $this
     */
    public function addFilter($filterName, $options = array())
    {
        ...
        // Add the filter
        $this->options['filters'][$filterHash] = array(
    ...
    protected function doApplyFilters(array $results, Server $server)
    {
        ...
        foreach ($this->options['filters'] as $filterOptions) {
            ...
            try {
                ...
                $filter = $class->newInstanceArgs(array($filterOptions['options']));
